...
def test_load_config_falls_back_to_legacy_path(
    ...
    config_file = tmp_path / "platform" / "config.toml"
    legacy_file = tmp_path / "legacy" / "config.toml"
    ...
    monkeypatch.setattr(config_module, "CONFIG_FILE", config_file)
    monkeypatch.setattr(config_module, "LEGACY_CONFIG_FILE", legacy_file)
...
def test_load_config_prefers_platform_path_over_legacy_path(
    tmp_path: Path,
    monkeypatch,
) -> None:
    config_file = tmp_path / "platform" / "config.toml"
